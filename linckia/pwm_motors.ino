...
static const uint8_t motor_phase_pins[num_motors] = {4, 12, 8, 7};
static const uint8_t motor_enpwm_pins[num_motors] = {9, 10, 5, 13};
...
void MotorCommand(byte command[6]) {
  ...
  bool motor_dir = command[2]; //0 or 1

  motor_target[motor] = motor_dir ? command[3] : -command[3]; // invert if necessary
...
void moveMotors() {
  for (int k = 0; k < 4; k++) //cycle through motor outputs
  {
    ...
    {
      if (motor_movtime[k] < ulActuatorsInterval) //if command time is less than 100 milisecond
      {
      ...
      {
        float motorspeed = ((float)(motor_target[k] - motor_current[k])*ulActuatorsInterval)/motor_movtime[k]; //calculated motor speed pwmchange/100ms
        motor_current[k] += (int)motorspeed; // update motor matrix

        moveMotor(k, motor_current[k]); // move motor at calculted speed for 10ms
        motor_movtime[k] = motor_movtime[k] - ulActuatorsInterval;
      }
    }
  }
}
